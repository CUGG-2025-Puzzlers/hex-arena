...
@onready var _chat_box: RichTextLabel = %ChatBox
...
func _on_copy_external_ip_pressed():
...
func _on_copy_internal_ip_pressed():
...
func _send_chat_message():
	...
	if msg.is_empty():
		return
	_chat_input.text = ""
	_receive_chat.rpc(msg)

# Delivers a chat message to all clients, including the sender
@rpc("call_local", "any_peer", "reliable")
func _receive_chat(msg: String):
	var sender_id := multiplayer.get_remote_sender_id()
	var sender_name := "Unknown"
	if sender_id in MultiplayerManager.players:
		sender_name = MultiplayerManager.players[sender_id].name
	elif sender_id == 0:
		sender_name = MultiplayerManager.player_info.name
	_chat_box.append_text("%s: %s\n" % [sender_name, msg])
