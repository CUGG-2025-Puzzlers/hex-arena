...
# Zilo's ordinary movement dash. This ability never grants damage immunity.
...
func _physics_process(delta: float) -> void:
	...
	dash_elapsed += delta
	var progress: float = dash_elapsed / dash_duration
	...
	var speed_factor: float = pow(1.0 - progress, dash_speed_curve)
	var speed: float = (dash_distance / dash_duration) * speed_factor * 2.0
