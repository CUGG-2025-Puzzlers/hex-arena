...
func activate() -> void:
	...
	print("[FINAL OBJECTIVE] activated")	
	
func _on_area_entered(area: Area2D) -> void:
	print("[FINAL OBJECTIVE] touched by area=", area.name)

	var player := _find_player_node(area)

	if player == null:
		print("[FINAL OBJECTIVE] not a player")
		return

	var entered_player_id = player.get("player_id")

	if entered_player_id != null and int(entered_player_id) == player_id:
		print("[FINAL OBJECTIVE] objective complete")
		_complete_objective()
